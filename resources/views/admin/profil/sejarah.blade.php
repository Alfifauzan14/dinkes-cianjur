        ...
        <div class="card card-outline card-success">
            <div class="card-header d-flex align-items-center justify-content-between" style="padding: 16px 20px; background-color: #FFFFFF; border-bottom: 1px solid #E2E8F0;">
                <span class="d-flex align-items-center" style="gap: 8px;">
                    <span class="material-icons text-success" style="font-size: 20px;">history_edu</span>
            ...
            <div class="card-body p-4">
                <form action="{{ route('admin.profil.update') }}" method="POST" enctype="multipart/form-data" id="profile-form">
                    ...
                    <div class="form-section-title">
                        <span class="material-icons text-success" style="font-size: 20px;">article</span>
                        <span>Uraian Sejarah</span>
                    </div>

                    <div class="form-group mb-3">
                        <label for="sejarah_title" class="font-weight-bold" style="font-size: 13px; color: #1E293B;">Judul Latar Belakang <span class="text-danger">*</span></label>
                        <input type="text" name="sejarah_title" id="sejarah_title" 
                            value="{{ old('sejarah_title', $profile->sejarah_title) }}" 
                            class="form-control @error('sejarah_title') is-invalid @enderror" required style="border-radius: 3px;">
                        @error('sejarah_title') <span class="invalid-feedback">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group mb-3">
                        <label for="sejarah_text_1" class="font-weight-bold" style="font-size: 13px; color: #1E293B;">Paragraf Sejarah 1 <span class="text-danger">*</span></label>
                        <textarea name="sejarah_text_1" id="sejarah_text_1" rows="4" class="form-control @error('sejarah_text_1') is-invalid @enderror" required style="border-radius: 3px;">{{ old('sejarah_text_1', $profile->sejarah_text_1) }}</textarea>
                        @error('sejarah_text_1') <span class="invalid-feedback">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group mb-3">
                        <label for="sejarah_text_2" class="font-weight-bold" style="font-size: 13px; color: #1E293B;">Paragraf Sejarah 2 <span class="text-danger">*</span></label>
                        <textarea name="sejarah_text_2" id="sejarah_text_2" rows="4" class="form-control @error('sejarah_text_2') is-invalid @enderror" required style="border-radius: 3px;">{{ old('sejarah_text_2', $profile->sejarah_text_2) }}</textarea>
                        @error('sejarah_text_2') <span class="invalid-feedback">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-section-title mt-4">
                        <span class="material-icons text-success" style="font-size: 20px;">image</span>
                        <span>Logo / Gambar Sejarah</span>
                    </div>

                    <div class="form-group mb-0">
                        <div class="d-flex align-items-center" style="gap: 16px;">
                            <div style="width: 100px; height: 75px; flex-shrink: 0; border: 1px solid #CBD5E1; border-radius: 3px; overflow: hidden; background: #F8FAFC; display: flex; align-items: center; justify-content: center; padding: 6px;">
                            ...
                            <div class="custom-file" style="font-size: 13px; flex: 1;">
                                ...
                                <label class="custom-file-label" for="sejarah_image" style="border-radius: 3px;">Unggah logo sejarah baru...</label>
                            </div>
                        </div>
