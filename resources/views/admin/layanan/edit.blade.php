...
<div class="card card-outline card-success">
    <div class="card-header d-flex flex-wrap align-items-center justify-content-between" style="gap: 12px; padding: 16px 20px; background-color: #FFFFFF; border-bottom: 1px solid #E2E8F0;">
        <span class="d-flex align-items-center" style="gap: 8px;">
            ...
            <span class="font-weight-bold card-title-label">Edit Layanan Terpadu</span>
        </span>
                ...
                <div class="col-lg-8 col-12">
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" id="type" class="custom-select @error('type') is-invalid @enderror" required>
                            <option value="" disabled>Pilih Segmen Penerima Layanan</option>
                            <option value="Warga" {{ old('type', $layanan->type) == 'Warga' ? 'selected' : '' }}>Layanan Untuk Warga (Masyarakat)</option>
                            <option value="Faskes" {{ old('type', $layanan->type) == 'Faskes' ? 'selected' : '' }}>Layanan Untuk Faskes (Puskesmas / RS / Klinik)</option>
                            <option value="Nakes" {{ old('type', $layanan->type) == 'Nakes' ? 'selected' : '' }}>Layanan Untuk Nakes (Dokter / Bidan / Perawat)</option>
                        </select>
                        ...
                        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(105px, 1fr)); gap: 10px; margin-top: 6px;">
                            @foreach($icons as $icon)
                                <label class="text-center icon-radio-label" style="border: 1px solid #CBD5E1; border-radius: 3px; padding: 10px 8px; display: flex; align-items: center; justify-content: center; flex-direction: column; gap: 6px; cursor: pointer; background-color: #F8FAFC; transition: all 0.2s ease;">
                                    <input type="radio" name="icon" value="{{ $icon }}" {{ old('icon', $layanan->icon) == $icon ? 'checked' : '' }} required style="display:none;" onchange="updateIconSelection()">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" rows="4" placeholder="Jelaskan gambaran umum tentang pelayanan kesehatan ini...">{{ old('description', $layanan->description) }}</textarea>
