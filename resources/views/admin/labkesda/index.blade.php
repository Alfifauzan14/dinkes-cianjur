...
<div class="card card-outline card-success">
    <div class="card-header d-flex flex-wrap align-items-center justify-content-between" style="gap: 12px;">
        <span class="d-flex align-items-center" style="gap: 8px;">
            ...
            <span class="font-weight-bold card-title-label">Kelola Layanan Labkesda</span>
        </span>
        <div class="d-flex align-items-center" style="gap: 8px;">
            <button type="button" id="btn-save-order" class="btn btn-sm btn-success" style="display: none;" onclick="saveNewOrder('labkesda')">
            ...
            <a href="{{ route('admin.labkesda.settings.edit') }}" class="btn btn-sm btn-outline-secondary">
                <span class="material-icons" style="font-size:16px;">contact_support</span> Edit Kontak &amp; Alamat
            </a>
            <a href="{{ route('admin.labkesda.create') }}" class="btn btn-sm btn-success">
                <span class="material-icons" style="font-size:16px;">add</span> Tambah Layanan Baru
            </a>
        </div>
    </div>
                        ...
                        <tr class="order-row" data-id="{{ $category->id }}">
                            ...
                            <td class="align-middle">
                                ...
                                @if($category->description)
                                    <div class="text-muted" style="font-size: 12px; margin-top: 2px; max-width: 420px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{ $category->description }}</div>
                                @endif
                            </td>
                            <td class="align-middle">
                                @if($category->badge_text)
                                    <span class="badge" style="background-color: #E6F7F0; color: #009966; padding: 4px 8px; border-radius: 3px; font-size: 12px; font-weight: 700;">{{ $category->badge_text }}</span>
                                @else
                                    <span class="text-muted" style="font-style: italic; font-size: 13px;">Tidak ada</span>
                                @endif
                            </td>
                            <td class="text-center align-middle">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center" style="padding: 48px; color: #94A3B8;">
                                <span class="material-icons" style="font-size: 48px; display: block; margin-bottom: 12px; color: #CBD5E1;">science</span>
                                <p style="font-size: 15px; font-weight: 600;">Belum ada data layanan Labkesda.</p>
                                <p class="text-muted" style="font-size: 13px;">Klik <strong>"Tambah Layanan Baru"</strong> untuk memulai.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
